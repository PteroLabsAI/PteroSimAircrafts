...
Every number in the files names its source; a number with no source is marked "chosen". Nothing has flown with
ArduPlane yet: the model is built from the design files, not calibrated against a flight.
...
Elevon throw: **+-30 deg per side, chosen**. No source gives the servo/linkage travel; 30 deg is the throw the
hover-authority numbers below are computed with. The printed cove allows 63 deg TE down / 65.5 deg TE up
(STEP clash limit).
...
- all-up mass 3.400 kg, CG x 0.2888 / z -0.0053 m (matches `Metrics.xml` EYEPOINT);
...
- drop from 5 cm onto the feet, engines off: no bounce, no explosion, stands on all three feet (WOW 1/1/1) with a
  0.55 deg elastic lean: the wing caps compress 2.4 mm under the whole weight while the fin ball, 290 mm away, carries
  W x 4.3 mm / 290 mm.
...
washed wing strips lift 5.1 N (15 % of the weight) toward the upper surface in still-air hover, -15.0 deg standing
elevon trim against the 22.4 mm thrust-line arm (half the chosen throw), 10 % differential thrust rolls toward the
weaker motor through the strips, pure spanwise wind leaves the alpha tables and beta derivatives at zero and gives
5.2 N of broadside drag with a 0.37 N m weathercock moment about the CG, 16 m/s trim at alpha 5.0 deg / elevator
-1.2 deg / throttle 0.40, Vs 11.55 m/s, short period 9.1 rad/s zeta 0.35. The hover authority is a bound, not a
...
What it adds: the wash normal force of the 3 deg-incidence wing in hover (4.9 N, 15 % of the weight, toward the
...
free-stream part is untouched (cruise trim elevator -1.27 deg). Consequences: hover pitch authority -0.0453 N m/deg
(-0.83 rad/s2 per deg), roll 0.141 N m/deg, standing elevon trim -15.0 deg (50 % of the chosen throw), implied
ArduPlane rate-loop crossover with the shipped gains 4.2 rad/s (pitch) and 6.9 (body roll). Nothing here validates
the authority, it bounds it (open question 10).
...
| Quantity | Value | How |
|---|---|---|
| Masses, CG, inertias | 3.4 kg, CG x 0.2888 z -0.0053; Mass.xml Ixx 0.243 Iyy 0.043 Izz 0.280 (empty x 1.337, about the empty CG); all-up 0.246 / 0.054 / 0.289 | STEP solids x 3MF slicer settings, datasheet point masses, tubes, battery, 0.686 kg unaccounted lump 60 mm belly-side with an airframe-like inertia share (both chosen) |
...
| Gear spring / damping | 6800 N/m, 227 N/m/s | quadtailsitter's 3000 / 100 (1.5 kg) x 3.4 / 1.5; drop-tested |
...
- Elevon throw +-30 deg (see Control mapping).
- The 0.686 kg unaccounted lump: 60 mm on the belly side of the nominal CG (the standing photo says the CG is inside
  the tripod, not by how much) and an inertia share equal to 0.337 x the empty tensor (its candidates, wiring, LEDs,
...
1. **CG height is a choice, not a measurement.** The all-up CG z -0.0053 rests on placing the 0.686 kg unaccounted
   lump 60 mm toward the belly; the photo only proves the CG is on the belly side of the wing-cap line (z -0.001).
   Anywhere between 2 and ~30 mm inside is compatible with it, and the thrust-line arm (hence the standing elevon
   trim, 15 deg of a chosen 30 deg throw) moves with it. A two-point weighing of the real aircraft would settle it.
2. **Elevon throw.** Only the geometric clash limit (63 / 65.5 deg) is known; the servo horn and pushrod are not
   in the STEP set and SERVO5/6 MIN/MAX 1000/2000 say nothing about degrees. +-30 deg is chosen. The hover pitch
   gain scales with it directly.
...
4. **Throttle against the param.** Hover: JSBSim needs duty 0.604 at 22.2 V (thrust fraction 0.41 in ArduPilot's
...
6. **Sideslip between the limits.** `Aero.xml` is exact at beta 0 (AVL) and beta 90 (broadside drag) and a smooth
   ...
   the roll JSBSim makes by applying the side force at the AERORP, 16.5 mm above the CG: -0.087 N m at 10 m/s spanwise
   (0.6 deg of aileron at the capped hover authority), rolling the belly toward the wind.
...
9. **The transition corridor is derived, not calibrated.** The washed strips see the undeflected jet plus the
   ...
   repository does.
10. **The trim elevon's side force and the jet turning efficiency.** The elevon lift increment acts about 78 mm behind
    the CG (65 mm behind the AERORP, Cm_de / CZ_de x c), so the -15.0 deg standing trim costs 8.7 N toward the belly
    (the wash lift of 4.9 N the other way cancels part of it, the net lean is 6 deg). Hover in wind at 5..10 m/s trims
    at 16..24 deg of the chosen 30 deg throw with 12..13 N of elevon force. With that force in the balance the 10 m/s
    case closes only at -66 deg with 37 N of elevon force, where the jets turned through the flap angle could give
    27 N: no balance exists inside the momentum bound, so the model says this aircraft cannot hover in a 10 m/s wind,
    which the author reports it does. One of the chosen numbers is therefore wrong; the suspects in order are the
    elevon throw, the 22.4 mm thrust-line height above the CG (open questions 1 and 2), the wash factor k and the
    post-stall centre of pressure of the free-stream wing. If the real throw is smaller than 30 deg, this is the
    first margin to check. The hover authority is an upper bound and the ArduPlane hover gains have not been
    re-derived against it; a tethered hover test measuring pitch acceleration per degree of elevon is the evidence
    that would set eta.
